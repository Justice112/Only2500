	...
	<h3 style="margin-top:20px"><?php echo __('考试相关学生'); ?></h3>
		...
		<?php foreach ($examStudentShips as $examStudentShip): ?>
			<tr>
				...
				<td class="actions">
					<?php echo $this->Form->postLink(__('删除'), array('controller' => 'ExamStudentShips', 'action' => 'delete', $examStudentShip['ExamStudentShip']['id']), array(), __('您确定要删除  %s 吗?', $examStudentShip['Student']['name'])); ?>
				</td>
			</tr>
		<?php endforeach; ?>
		</table>
...
<div class="actions">
	...
	<ul>
		<li><?php echo $this->Form->postLink(__('删除该考试'), array('action' => 'delete', $this->Form->value('Exam.id')), array(), __('您确定要删除 # %s吗?', $this->Form->value('Exam.id'))); ?></li>
		<li><?php echo $this->Html->link(__('考试列表'), array('action' => 'index')); ?></li>
