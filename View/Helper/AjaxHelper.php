...
class AjaxHelper extends AppHelper {
	 ...
	 *        	message. Calls up a JavaScript confirm() message.
	 * @param boolean $escapeTitle
	 *        	Escaping the title string to HTML entities
	 *        	
	 * @return string code for link to remote action
	 */
	function link($title, $href = null, $options = array(), $confirm = null, $escapeTitle = true) {
		...
		$htmlOptions = $this->__getHtmlOptions ( $options, array (
				'url' 
		) );
		
		$htmlDefaults = array (
				'id' => 'link' . intval ( mt_rand () ),
				'onclick' => '' 
		);
	 ...
	 *
	 * @param mixed $params
	 *        	Either a string identifying the form target, or an array of method parameters, including:
	...
	function div($id, $options = array()) {
		...
		$attr = $this->_parseAttributes ( array_merge ( $options, array (
				'id' => $id 
		) ) );
	 ...
	 * For a reference on the options for this function,
	 ...
	 *
	 * @param string $id
	 *        	DOM ID of resizable handle
	 * @param array $options
	 *        	Array of options to control the resizable
	 ...
	 *
	 * @param string $id
	 *        	DOM ID of selectable handle
	 * @param array $options
	 *        	Array of options to control the selectable
	 ...
	 *
	 * @param string $id
	 *        	DOM ID of accordion handle
	 * @param array $options
	 *        	Array of options to control the accordion
	 ...
	 *
	 * @param string $id
	 *        	DOM ID of button handle
	 * @param array $options
	 *        	Array of options to control the button
	 ...
	 *
	 * @param string $id
	 *        	DOM ID of buttonset handle
	 * @param array $options
	 *        	Array of options to control the buttonset
	 * @link http://docs.jquery.com/UI/Button
	 */
	 ...
	 *
	 * @param string $id
	 *        	DOM ID of datepicker handle
	 * @param array $options
	 *        	Array of options to control the datepicker
	 ...
	 *
	 * @param string $id
	 *        	DOM ID of dialog handle
	 * @param array $options
	 *        	Array of options to control the dialog
	 ...
	 *
	 * @param string $id
	 *        	DOM ID of progressbar handle
	 * @param array $options
	 *        	Array of options to control the progressbar
	 ...
	 *
	 * @param string $id
	 *        	DOM ID of tabs handle
	 * @param array $options
	 *        	Array of options to control the tabs
	 ...
	 * @param array $options
	 *        	Array of options to control the editor (see link).
		...
		foreach ( $options as $key => $value ) {
			switch ($key) {
				...
				case 'form' :
					...
					break;
				default :
					if (! in_array ( $key, $this->callbacks ) && ! in_array ( $key, array (
							'before',
							'after',
							'confirm',
							'condition',
	 ...
	 *
	 * @param array $options
	 *        	in the shape of keys and values
	 * @param array $extra
	 *        	of legal keys in this options context
	 ...
	 *        	of legal keys in this options context
	 * @return string of Javascript array definition
	 */
		...
		foreach ( $this->callbacks as $callback ) {
			if (isset ( $options [$callback] )) {
				...
				if (isset ( $options ['bind'] )) {
					...
					$hasBinding = ((is_array ( $bind ) && in_array ( $callback, $bind )) || (is_string ( $bind ) && strpos ( $bind, $callback ) !== false));
	 ...
	 * @param array $options
	 *        	options array
	 * @param array $stringOpts
	 *        	as strings in an array
	 * @access private
	 * @return array
	 */
	function _optionsToString($options, $stringOpts = array()) {
		foreach ( $stringOpts as $option ) {
			$hasOption = (isset ( $options [$option] ) && ! empty ( $options [$option] ) && is_string ( $options [$option] ) && $options [$option] [0] != "'");
