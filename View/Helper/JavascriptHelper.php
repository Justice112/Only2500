...
class JavascriptHelper extends AppHelper {
	/**
	 * Determines whether native JSON extension is used for encoding.
	...
	var $enabled = true;
	/**
	 * Indicates whether <script /> blocks should be written 'safely,' i.e.
	...
	var $_cacheAll = false;
	/**
	 * Contains event rules attached with CSS selectors.
	 * Used with the event:Selectors JavaScript
	 * library.
	 *
	 * @var array
	 * @access protected
	...
	/**
	 * Constructor.
	 * Checks for presence of native PHP JSON extension to use for object encoding
	 *
	 * @access public
	 */
	 ...
	 *        	If true, the <script /> tag will be printed inline,
	...
	function event($object, $event, $observer = null, $options = array()) {
		$isObject = (strpos ( $object, 'window' ) !== false || strpos ( $object, 'document' ) !== false || strpos ( $object, '$(' ) !== false || strpos ( $object, '"' ) !== false || strpos ( $object, '\'' ) !== false);
		...
		if ($this->useNative) {
			...
			foreach ( $data as $key => $val ) {
				if (is_array ( $val ) || is_object ( $val )) {
					$val = $this->object ( $val, array_merge ( $options, array (
							'block' => false 
					) ) );
				} else {
					$quoteStrings = (! count ( $options ['stringKeys'] ) || ($options ['quoteKeys'] && in_array ( $key, $options ['stringKeys'], true )) || (! $options ['quoteKeys'] && ! in_array ( $key, $options ['stringKeys'], true )));
	 ...
	 * Writes any cached events to the view, or to a temp file.
	 *
	 * @return null
	 */
